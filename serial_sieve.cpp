...
class SearchInterval {
	...
	public:
		SearchInterval(uint64_t lowerbound, uint64_t upperbound) 
		{
			state = SIS_NEW;
			set_extrema_safely(lowerbound, upperbound);
			internal_storage = (uint64_t*)malloc(capacity * sizeof(uint64_t));
			initialize();
		};
		~SearchInterval() {
		...
		friend std::ostream& operator<< (std::ostream& o, const SearchInterval& si) {
